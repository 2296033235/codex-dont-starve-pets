"""Objective QA for the WX-78 atlas: locomotion facing, mirroring, and the
16 look directions (dark eye-socket feature displacement vs the neutral cell)."""
...
def facing_of(img):
    """Return (eye_x, head_center_x, side) for a WX-78 side-view cell.

    WX-78's head is wrapped in a thick black outline, so any measure built on
    "how much dark pixel mass is there" just measures the outline and reads the
    middle.  The eye screen, however, is the one *solid* dark blob on the head,
    so this picks the dark connected component with the highest fill ratio
    (area / bounding-box area) inside the head band and uses its centroid.
    """
    ...
    for y0, x0 in zip(ys, xs):
        ...
        while stack:
            ...
            for ny, nx in ((y - 1, x), (y + 1, x), (y, x - 1), (y, x + 1)):
                if 0 <= ny < dark.shape[0] and 0 <= nx < dark.shape[1] \
                        and dark[ny, nx] and not seen[ny, nx]:
                    seen[ny, nx] = True
                    stack.append((ny, nx))
        area = len(pix)
        if area < 120:
        ...
        if best is None or fill > best[0]:
            best = (fill, sum(px) / area, area, fill)
    if best is None:
        return None
    _, eye_x, area, fill = best
    ys2, xs2 = np.nonzero(alpha)
    head_cx = float(xs2.mean())
    return eye_x, head_cx, ("RIGHT" if eye_x > head_cx else "LEFT")


def eye_feature_map(img):
    """Darkness of the head band: WX-78's eye sockets are the landmark."""
    a = np.asarray(img).astype(np.float32)
    alpha = a[:, :, 3] > 200
    lum = a[:, :, :3].mean(axis=2)
    feat = np.where(alpha, np.clip(160.0 - lum, 0.0, None), 0.0)
...
def main() -> None:
    ...
    for row, label in [(1, "running-right"), (2, "running-left"),
        ...
        if res:
            feat_x, shell_x, side = res
            report["facing"][f"row{row}"] = {
                "feature_x": round(float(feat_x), 1),
                "shell_x": round(float(shell_x), 1),
                "faces": side,
            }
            print(f"row {row}: feature_x={feat_x:6.1f} shell_x={shell_x:6.1f} -> {side}")
    ...
    for rec in report["gaze"]:
        ...
        if name:
            ...
            verdicts.append({"direction": f"{deg:g}", "expected": name,
                             ...
                             "reason": f"eye-socket features moved {cdx:+d}px x, "
    ...
    with open(os.path.join(RUN, "qa", "direction-semantics.json"), "w", encoding="utf-8") as fh:
        json.dump({"method": "eye-socket feature SSD alignment against the neutral cell (row 0 col 6)",
