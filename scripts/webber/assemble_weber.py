...
Don't Starve animation frames for Weber, plus a 16-pose look loop built from
...
WEBBER = os.path.join(DS, "webber.zip")
BUILD = kanim.load_build_from_zip(WEBBER)
ATLAS = ra.load_atlas(WEBBER)

HEAD_PARTS = {"headbase", "headbase_hat", "face", "cheeks", "hairpigtails",
              "lens_eye", "SWAP_FACE"}
...
ROW_SPECS = [
    ...
    # The client forces this row on pointer-enter.  Weber has no character-specific
    # idle in the single-player install, so the hover row carries the official
    # hot-weather idle: a calm wiping-sweat loop that stays the same size as idle.
...
def sample_indices_normal_height(anim, wanted):
    boxes = []
...
def main() -> None:
    ...
    for row, (name, wanted, spec, explicit) in enumerate(ROW_SPECS):
        ...
        if explicit is not None:
            ...
            idx = sample_indices_normal_height(anim, wanted)
        else:
    ...
    provenance["_notes"] = {
        ...
        "row_4_note": (
            "the client forces the jumping row on pointer-enter (hover); Weber has "
