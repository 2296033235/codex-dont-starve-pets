"""Extract Weber's official art from the single-player Don't Starve install."""
...
def main() -> None:
    part_dir = os.path.join(OUT, "parts_weber")
    os.makedirs(part_dir, exist_ok=True)
    zip_path = os.path.join(DS, "data", "anim", "webber.zip")
    with zipfile.ZipFile(zip_path) as z:
        data = z.read("atlas-0.tex")
    img = ktex.decode_mipmap(data, 0)[::-1, :]
    Image.fromarray(img, "RGBA").save(os.path.join(OUT, "weber_atlas_ds.png"))
    pal = palette_of(img)
    ...
    with open(os.path.join(OUT, "palette_weber.json"), "w", encoding="utf-8") as fh:
    ...
    sheet.save(os.path.join(OUT, "parts_contact_weber.png"))

    report = {"atlas": "weber_atlas_ds.png", "parts": len(boxes),
